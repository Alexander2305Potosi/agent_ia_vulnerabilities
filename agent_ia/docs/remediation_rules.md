...
- **Centralización en Raíz (Root-Only ext)**: Cualquier variable de versión (`ext`) debe residir exclusivamente en el `build.gradle` de la raíz del microservicio. El agente tiene prohibido crear bloques `ext` en subcarpetas; cualquier definición encontrada en subniveles será purgada y migrada a la raíz para mantener la coherencia arquitectónica.
